...
## The `ExcitationModel` interface

Again, this is extremely similar to the `IonizationModel` interface. There are two functions to define: `supported_species` and `load_reactions`. These function identically to those used in the `IonizationModel` interface, with `load_reactions` returning a vector of `ExcitationReaction`s instead of `IonizationReaction`s. We do not need to define a `maximum_charge_state` function here, as HallThruster.jl will just load whatever excitation reactions it can find for the provided species. This means we do not need to provide excitation reactions for every species.
