...
Users implementing their own `WallLossModel` will need to implement at least three methods
    ...
    2) `wall_electron_current(model, U, params, i`: Compute the electron current to the walls in cell `i`
...
function wall_electron_current(model::WallLossModel, U, params, i)
    error("wall_electron_current not implemented for wall loss model of type $(typeof(model)). See documentation for WallLossModel for a list of required methods")
end
