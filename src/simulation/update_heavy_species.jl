function iterate_heavy_species!(dU, U, params; apply_boundary_conditions = true)
    ...
    # user-provided continuity source terms
    if !any(isnothing, source_ion_continuity)
        @inbounds for i in 2:(ncells - 1), Z in 1:ncharge
            dU[index.ρi[Z], i] += source_ion_continuity[Z](U, params, i)
        end
    end

    # user-provided momentum source terms
    if !any(isnothing, source_ion_momentum)
        @inbounds for i in 2:(ncells - 1), Z in 1:ncharge
