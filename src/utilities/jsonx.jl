# Simplified vendorable version of JSON.jl from https://github.com/JuliaIO/JSON.jl/blob/master/vendor/jsonx.jl
# Modified by Thomas Marks for HT.jl needs
module JSONX
...
function write_json(io::IO, value; replace_nan = false, replace_inf = false)
    ...
    elseif value isa Number
        ...
        if !isfinite(value)
            if (isinf(value) && replace_inf) || (isnan(value) && replace_nan)
                value = zero(typeof(value))
            else
