...
@testset "Restarts" begin
    ...
    config = het.Config(;
        ...
        wall_loss_model = het.WallSheath(het.BoronNitride),
        transition_length = 1e-3,
    )

    ncells = 50
    grid = het.generate_grid(
        config.thruster.geometry, config.domain, het.EvenGrid(ncells),)
    fluids, fluid_ranges, species, species_range_dict = het.configure_fluids(config)

    #make a new restart from the current configuration
    duration = 1e-7
    dt = 1e-8
    nsave = 2
    sol = het.run_simulation(config; ncells, duration, dt, nsave, verbose = false)
    het.write_restart(restart_path, sol)

    # Check that writing a restart of a restart does not lose any information
    # i.e. restarts are perfectly reconstructable
    restart = het.read_restart(restart_path)
    ...
    U, cache = het.load_restart(grid, config, sol)
    U2, cache2 = het.load_restart(grid, config, restart_path)
    ...
    restart_sol = het.run_simulation(
        config; dt = 5e-9, duration = 4e-9, grid = het.EvenGrid(ncells),
...
@testset "Landmark data loading" begin
    landmark_1_1 = het.load_landmark_data(1, "fluid_1")
    landmark_1_2 = het.load_landmark_data(1, "fluid_2")
    landmark_1_hybrid = het.load_landmark_data(1, "hybrid")

    landmark_2_1 = het.load_landmark_data(2, "fluid_1")
    landmark_2_2 = het.load_landmark_data(2, "fluid_2")
    landmark_2_hybrid = het.load_landmark_data(2, "hybrid")

    landmark_3_1 = het.load_landmark_data(3, "fluid_1")
    landmark_3_2 = het.load_landmark_data(3, "fluid_2")
    landmark_3_hybrid = het.load_landmark_data(3, "hybrid")

    @test length(landmark_1_1.u) == 1
    @test size(landmark_1_1.u[1]) == (4, 100)
    @test all(isnan, landmark_1_1.u[1][3, :])

    landmark_1 = het.load_landmark_data(1)
