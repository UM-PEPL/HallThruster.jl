...
function run_sim_energytest(end_time = 0.0002, n_save = 2)
    ...
    function affect!(integrator)
        ...
        @inbounds for i in 1:(ncells + 2) #pay attention as to whether J or eV in electron energy equ. 
            #update electron temperature from energy using old density
            U[index.ne, i] = max(1e-13, HallThruster.electron_density(@view(U[:, i]), fluid_ranges) / fluid.m)
            ...
            U[index.ue, i] = min(-100.0, HallThruster.electron_velocity(U, params, i))
            #@show U[index.ue, i]
    ...
    sim = HallThruster.MultiFluidSimulation(
        grid = HallThruster.generate_grid(HallThruster.SPT_100, 50),
