    ...
    @testset "$(file)" begin
        ...
        for (field_name, field_fn) in zip(field_names, field_fns)
            ...
            for (reduction_name, reduction) in zip(reduction_names, reductions)
                if field_name == "ni_3" && reduction == "max"
                    continue # For some reason this gives issues in CI for Julia 1.12 when coverage is on. Re-enable this as soon as possible.
                end
