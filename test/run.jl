...
function animate_solution(sol, saved_values)
    ...
    @gif for (u, t) in zip(sol.u, sol.t) #uₑ
        p = plot(ylims = (-1e6, 1e6))
        plot!(p, u[7, :], yaxis = :log)
    end
    @gif for (u, t) in zip(sol.u, sol.t) #- grad_ϕ
        p = plot(ylims = (0, 5e5))
        plot!(p, u[8, :])
    end
    @gif for (u, t) in zip(sol.u, sol.t) # grad_pe/e/params.cache.ne[i]
        p = plot(ylims = (0, 5e5))
        plot!(p, u[7, :], yaxis = :log)
    end
end